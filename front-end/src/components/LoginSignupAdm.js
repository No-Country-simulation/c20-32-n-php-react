      ...
      fetch("https://cursotekaba.zeabur.app/api/usuario/") // Fetch all users ubuntu 22 linux 
        ...
        .then((data) => {
          ...
          if (!user) {
            ...
            if (user.password === password && user.rol==="administrador") {
              alert('Credenciales de administrador correctas');
              sessionStorage.setItem('username', username);
              sessionStorage.setItem('userrole', user.rol);
              usenavigate('/Principal_Configuracion');
            } else {
    ...
    crossorigin="anonymous" referrerpolicy="no-referrer" /> 
      ...
      <div className="form-container sign-up-container">
        <form onSubmit={ProceedInscribir} >
          <h1>Crear una cuenta de Administrador</h1>
